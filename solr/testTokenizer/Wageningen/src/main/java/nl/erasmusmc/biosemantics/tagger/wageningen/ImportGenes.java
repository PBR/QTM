...
public class ImportGenes {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String core = line.hasOption("core") ? line.getOptionValue("core") : "SGN";
	...
	private static void processDictionary(String server, String core, String filename, Boolean permutations,
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				String geneUri = geneinfo[0];
				String geneName = geneinfo[1];
			
						if (permutations) {
							SolrInputDocument doc = new SolrInputDocument();
							doc.addField("uuid", UUID.randomUUID().toString());
							doc.addField("code", geneUri);
							doc.addField("term", geneName);
							doc.addField("origin", "SGN_graph");
							doc.addField("prefterm", geneName);
							docs.add(doc);
						} else {
							SolrInputDocument doc = new SolrInputDocument();
							doc.addField("uuid", UUID.randomUUID().toString());
