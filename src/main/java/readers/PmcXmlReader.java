...
public class PmcXmlReader implements Reader{
	...
	public Article ParseMetaData(Article art, Document parse, String xml)
	{
	    ...
	    for(int j=0;j<article_id.getLength();j++)
	    {
	    	...
	    	if(article_id.item(j).getAttributes()!=null && article_id.item(j).getAttributes().getNamedItem("pub-id-type")!=null && article_id.item(j).getAttributes().getNamedItem("pub-id-type").getNodeValue().equals("pmcid"))
	    	{
