	...
	private static void processDictionary(String server, String core, String filename, Boolean permutations,
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				String[] potatoTraitinfo = line.split(cvsSplitBy);

				String potatoTraitUri = potatoTraitinfo[0];
				String potatoTraitName = potatoTraitinfo[1];
				//String[] mNames = makerName.split(";");
				
						if (permutations) {
							SolrInputDocument doc = new SolrInputDocument();
							doc.addField("uuid", UUID.randomUUID().toString());
							doc.addField("code", potatoTraitUri);
							doc.addField("term", potatoTraitName);
							doc.addField("origin", "CO_330_dictionary");
							doc.addField("prefterm", potatoTraitName);
							docs.add(doc);
						} else {
							SolrInputDocument doc = new SolrInputDocument();
							doc.addField("uuid", UUID.randomUUID().toString());
